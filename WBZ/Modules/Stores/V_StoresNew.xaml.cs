...
namespace WBZ.Modules.Stores
{
	...
	public partial class StoresNew : New
	{
		...
		private void tabs_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			...
			if (tab?.Name == "tabSources_Articles")
			{
				if (D.InstanceInfo.ID != 0 && D.InstanceSources_Articles == null)
					D.InstanceSources_Articles = SQL.ListInstances<M_Article>(M_Module.Module.ARTICLES, $"sa.store={D.InstanceInfo.ID}");
			}
			else if (tab?.Name == "tabSources_Documents")
			{
				if (D.InstanceInfo.ID != 0 && D.InstanceSources_Documents == null)
					D.InstanceSources_Documents = SQL.ListInstances<M_Document>(M_Module.Module.DOCUMENTS, $"d.store={D.InstanceInfo.ID}");
			}
		}
		...
		{
			dgList_Module_MouseDoubleClick<M_Article>(sender, e, M_Module.Module.ARTICLES);
		}
