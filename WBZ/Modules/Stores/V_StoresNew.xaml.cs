			...
			await Task.Run(() =>
			{
				if (tab?.Name == "tabSources_Articles")
				{
					if (D.InstanceData.ID != 0 && D.InstanceSources_Articles == null)
						D.InstanceSources_Articles = SQL.ListInstances<M_Article>(Config.GetModule(nameof(Articles)), $"sa.store={D.InstanceData.ID}");
				}
				else if (tab?.Name == "tabSources_Documents")
				{
					if (D.InstanceData.ID != 0 && D.InstanceSources_Documents == null)
						D.InstanceSources_Documents = SQL.ListInstances<M_Document>(Config.GetModule(nameof(Documents)), $"d.store={D.InstanceData.ID}");
				}
			});
		}
