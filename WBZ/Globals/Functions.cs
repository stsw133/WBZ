    ...
    internal static class Functions
    {
		...
		*/
		...
		internal static void OpenInstanceWindow<T>(Window owner, T instance, Commands.Type mode)
		{
			...
			if (SQL.CountInstances(mm.Module, $"{mm.Module.Tag}.id={mm.Instance}") == 0)
				return;
			...
			switch (mm.Module.Name)
			{
				...
				case nameof(Modules.Articles):
					window = new ArticlesNew(SQL.GetInstance<M_Article>(mm.Module, mm.Instance), mode);
					break;
				/// ATTRIBUTES_CLASSES
				case nameof(Modules.AttributesClasses):
					window = new AttributesClassesNew(SQL.GetInstance<M_AttributeClass>(mm.Module, mm.Instance), mode);
					break;
				/// CONTRACTORS
				case nameof(Modules.Contractors):
					window = new ContractorsNew(SQL.GetInstance<M_Contractor>(mm.Module, mm.Instance), mode);
					break;
				/// DISTRIBUTIONS
				case nameof(Modules.Distributions):
					window = new DistributionsNew(SQL.GetInstance<M_Distribution>(mm.Module, mm.Instance), mode);
					break;
				/// DOCUMENTS
				case nameof(Modules.Documents):
					window = new DocumentsNew(SQL.GetInstance<M_Document>(mm.Module, mm.Instance), mode);
					break;
				/// EMPLOYEES
				case nameof(Modules.Employees):
					window = new EmployeesNew(SQL.GetInstance<M_Employee>(mm.Module, mm.Instance), mode);
					break;
				/// FAMILIES
				case nameof(Modules.Families):
					window = new FamiliesNew(SQL.GetInstance<M_Family>(mm.Module, mm.Instance), mode);
					break;
				/// ICONS
				case nameof(Modules.Icons):
					window = new IconsNew(SQL.GetInstance<M_Icon>(mm.Module, mm.Instance), mode);
					break;
				/// STORES
				case nameof(Modules.Stores):
					window = new StoresNew(SQL.GetInstance<M_Store>(mm.Module, mm.Instance), mode);
					break;
				/// USERS
				case nameof(Modules.Users):
					window = new UsersNew(SQL.GetInstance<M_User>(mm.Module, mm.Instance), mode);
					break;
				/// VEHICLES
				case nameof(Modules.Vehicles):
					window = new VehiclesNew(SQL.GetInstance<M_Vehicle>(mm.Module, mm.Instance), mode);
					break;
				default:
					return;
			}
